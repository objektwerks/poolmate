...
case class Addition(id: Int = 0, poolid: Int, on: LocalDate = LocalDate.now, tablets: Int = 0, ch: Int = 0)
...
object Entity {
  ...
  implicit def additionOrdering: Ordering[Addition] = Ordering.by(_.on)
}